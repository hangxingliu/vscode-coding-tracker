...
function encodeVCSInfo(fileName, repoPath, branch) {
	if (!repoPath) return log.debug(`Can not find any vcs info of document: ${fileName}`), null;
	if (!branch) return log.warn(`The vcs "${repoPath}" has not branch information!`), null;
